...
> **Split module.** The root `z` is generated by `bin/zbundle` from the parts
> in `lib/z/`; edit a part and run `make bundle`. See
> [Split modules](../architecture.md#split-modules).
...
z::sys::die "Unrecoverable error" $Z_ERR_GENERAL
```
...
| `Z_ERR_*` | Shared error codes, defined by zbase (`Z_ERR_GENERAL`, `Z_ERR_INPUT`, `Z_ERR_NOTFOUND`, `Z_ERR_PERM`) |
| `ZCORE_ERROR_*` | z-only codes: `ZCORE_ERROR_TIMEOUT`, `ZCORE_ERROR_INTERRUPTED` |
...
| `Z_ERR_GENERAL` | `1` | Unspecified failure (defined by zbase) |
| `Z_ERR_INPUT` | `2` | Bad argument type, format, or value |
| `Z_ERR_NOTFOUND` | `3` | Cache miss, missing config key, unreadable file |
...
|---|---|---|
| `ZCORE_VERSION` | string (`"0.6.0"`) | Framework version; matches `./VERSION` |
...
**Returns:** `0` on success; `Z_ERR_INPUT` on missing key or invalid TTL.
...
**Returns:** `0` on hit; `Z_ERR_NOTFOUND` on miss or expired entry;
`Z_ERR_INPUT` when key is empty.
...
**Returns:** `0` on success; `Z_ERR_INPUT` when key is empty.
...
**Returns:** `0` on success; `Z_ERR_INPUT` on missing key/func;
`Z_ERR_NOTFOUND` when func is undefined; propagates func exit code on failure.
...
**Returns:** `0` on success; `Z_ERR_INPUT` on type mismatch or
empty key.
...
**Returns:** `0` on success; `Z_ERR_INPUT` when path is empty.
...
**Returns:** `0` on success; `Z_ERR_INPUT` when path is empty;
`Z_ERR_NOTFOUND` when the file is missing or unreadable.
...
| `exit-code` | integer | `$Z_ERR_GENERAL` | Exit/return code |

Clears any active progress bar first (safely skipped when `ui` is not loaded).

The exit code is validated: it must be an integer in `1..255`. A missing,
non-numeric, zero, or out-of-range value is rejected with a warning and
`$Z_ERR_GENERAL` is used instead — a fatal error must never report success.

**Returns/Exits:** In an **interactive** shell the code is *returned*, so a
fatal error never closes the user's session; the caller is responsible for
unwinding. In a **non-interactive** shell (scripts) it calls `exit <code>`.
...
**Returns:** `0` on success; `Z_ERR_GENERAL` if the profiling store cannot be opened.
...
**Returns:** `0` on success; `Z_ERR_NOTFOUND` when no start time exists.
...
|---|---|---|
| `ZCORE_VERSION` | Constants | Framework version (`0.6.0`) |
