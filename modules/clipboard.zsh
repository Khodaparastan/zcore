...
cpfile()
{
  ...
  elif z::probe::cmd "clip.exe"; then # WSL
    clipboard_cmd="clip.exe"
  elif [[ -n "${WAYLAND_DISPLAY:-}" ]] \
    && z::probe::cmd "wl-copy"; then
    clipboard_cmd="wl-copy"
  elif [[ -n "${DISPLAY:-}" ]] \
    && z::probe::cmd "xclip"; then
    clipboard_cmd="xclip -selection clipboard -in"
  elif z::probe::cmd "tmux"; then
...
# MAIN INITIALIZATION
# ==============================================================================
__z::mod::clipboard::init()
{
  emulate -L zsh
  z::log::info "Clipboard tools (cpfile) are now available."
}

if z::probe::func "__z::mod::clipboard::init"; then
  __z::mod::clipboard::init
fi
