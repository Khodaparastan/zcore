...
_install_zi() {
    declare -g ZI_HOME="${ZI_HOME:-$XDG_DATA_HOME/zi}"
    declare -g ZI_BIN_DIR="$ZI_HOME/bin"
    declare -g ZI_SCRIPT="$ZI_BIN_DIR/zi.zsh"
    ...
    if ! _check_network 3; then
        print -u2 -P "%F{yellow}Warning: No network connectivity. Skipping ZI installation.%f"
        return 1
    fi

    print -P "%F{cyan}Installing ZI plugin manager to $ZI_HOME...%f"
    if ! mkdir -p -m 755 "$ZI_BIN_DIR" 2>/dev/null; then
        print -u2 -P "%F{red}Error: Cannot create ZI directory: $ZI_BIN_DIR%f"
        return 1
    fi
    ...
    "https://github.com/z-shell/zi.git" "$ZI_BIN_DIR" >/dev/null 2>&1; then
        if [[ -f "$ZI_SCRIPT" ]]; then
            source "$ZI_SCRIPT"
            print -P "%F{green}ZI installed successfully.%f"
            return 0
        else
            print -u2 -P "%F{red}Error: ZI cloned, but zi.zsh script not found at $ZI_SCRIPT%f"
        fi
    else
        print -u2 -P "%F{red}Error: ZI installation (git clone) failed.%f"
    fi
