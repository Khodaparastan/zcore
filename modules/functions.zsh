...
# Multi-part extensions used by helpers for base-name and unique-path logic.
# Keep the longest-first order to ensure correct stripping.
typeset -ga __MULTI_EXTS=(
  .tar.gz
  .tar.bz2
  .tar.xz
  .tar.zst
...
extract() {
  ...
  local FAILC=""
  (( failed_count > 0 )) && FAILC=$'\033[31m'
  printf "  %sFailed: %d${RST}\n" "$FAILC" "$failed_count"
...
backup() {
  ...
  while getopts ":d:cz:L:e:R:n:Vh" opt; do
    case "$opt" in
      ...
      z)
        ...
        ;;
      L)
        if [[ "$OPTARG" =~ ^[0-9]+$ ]] && (( OPTARG >= 0 && OPTARG <= 22 )); then
          comp_level="$OPTARG"
        else
          print -u2 "Error: Compression level must be a number between 0-22"
          return 1
        fi
        ;;
      e)
      ...
      h)
        ...
        printf "  ${CYN}-L${RST} LEVEL     Compression level (format-dependent, gz 1–9; zst 1–22; xz 0–9)\n"
  ...
  for item in "$@"; do
    ...
    if ((compress_backup)); then
      ...
      case "$comp_format" in
        ""|"gz")
          ...
          # gzip level validated/clamped after case
          ;;
        "zst"|"zstd")
          ext=".tar.zst"
      ...
      # Per-format compression level validation/clamping
      case "$ext" in
        ".tar.gz")
          if [[ -n "$comp_level" ]]; then
            if [[ "$comp_level" -lt 1 || "$comp_level" -gt 9 ]]; then
              print -u2 "Warning: gzip level $comp_level out of range (1-9); clamping."
              (( comp_level = comp_level < 1 ? 1 : 9 ))
            fi
            comp_cmd+=(-"$comp_level")
          fi
          ;;
        ".tar.zst")
          if [[ -n "$comp_level" ]]; then
            if [[ "$comp_level" -lt 1 || "$comp_level" -gt 22 ]]; then
              print -u2 "Warning: zstd level $comp_level out of range (1-22); clamping."
              (( comp_level = comp_level < 1 ? 1 : 22 ))
            fi
            comp_cmd=(zstd -T0 -"$comp_level" -q)
          fi
          ;;
        ".tar.xz")
          if [[ -n "$comp_level" ]]; then
            if [[ "$comp_level" -lt 0 || "$comp_level" -gt 9 ]]; then
              print -u2 "Warning: xz level $comp_level out of range (0-9); clamping."
              (( comp_level = comp_level < 0 ? 0 : 9 ))
            fi
            comp_cmd+=(-"$comp_level")
          fi
          ;;
    ...
    if [[ "$retain_n" =~ ^[0-9]+$ && "$retain_n" -gt 0 ]]; then
        ...
        local qprefix
        qprefix="${(q)prefix}"
        backups=("$backup_dir_base"/${qprefix}*(NOm))
...
local distro
distro="$(system_profiler SPSoftwareDataType 2>/dev/null | awk '/System Version/ {print $3, $4; exit}' || echo 'N/A')"
[[ -z "$distro" ]] && distro="N/A"
...
# Linux section
local distro
distro="$(lsb_release -d 2>/dev/null | cut -f2 || cat /etc/os-release 2>/dev/null | awk -F'=' '/^PRETTY_NAME/ {gsub(/"/, "", $2); print $2; exit}' || echo 'N/A')"
[[ -z "$distro" ]] && distro="N/A"
printf "Distribution: ${CYN}%s${RST}\n" "$distro"
...
;;
-h|--help)
_qfind_help; _qfind_examples; return 0
;;
-*)
...
if [[ "$mode" == "regex" ]]; then
prod_cmd+=(--regex -- "$pattern" "$search_dir")
else
# Auto-wrap pattern unless exact mode or already contains wildcards
if (( !exact )) && [[ "$pattern" != *'*'* && "$pattern" != *'?'* ]]; then
pattern="*${pattern}*"
fi
prod_cmd+=(--glob -- "$pattern" "$search_dir")
fi
if ((zero_delim)); then
prod_cmd+=(-0)
fi
...
if [[ "$mode" == "regex" ]]; then
prod_cmd+=(-iregex ".*${pattern}.*")
else
# Auto-wrap pattern unless exact mode or already contains wildcards
if (( !exact )) && [[ "$pattern" != *'*'* && "$pattern" != *'?'* ]]; then
pattern="*${pattern}*"
fi
prod_cmd+=(-iname "$pattern")
fi
if ((zero_delim)); then
prod_cmd+=(-print0)
else
