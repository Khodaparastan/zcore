...
function _is_poetry_shell_active
{
  ...
  [[ -n "$POETRY_ACTIVE" ]] \
    || [[ -n "$VIRTUAL_ENV" && "$VIRTUAL_ENV" == *"poetry"* ]]
}
...
function rmvenv
{
  ...
  if [[ "$venv_name" == ".venv" ]] \
    ...
    if command -v poetry > /dev/null 2>&1; then
      poetry env list 2> /dev/null \
        || z::log::info "  No Poetry environments found"
    fi
    __PYTHON_VENV_OPERATION=0
    return 1
  fi
...
function lsvenv
{
  ...
  if _is_poetry_project; then
    ...
    if command -v poetry > /dev/null 2>&1; then
      local poetry_envs
      if poetry_envs=$(poetry env list --full-path 2> /dev/null) \
        && [[ -n "$poetry_envs" ]]; then
        ...
        while IFS= read -r line; do
          if [[ -n "$line" ]]; then
            ...
            if [[ "$line" == *"(Activated)"* ]] \
              || [[ -n "$VIRTUAL_ENV" && "$VIRTUAL_ENV" == "$env_path" ]]; then
              env_status="[active]"
            fi
  ...
  if ((${#venv_dirs[@]} > 0)); then
    ...
    for dir in ${(o)venv_dirs}; do
      ...
      if [[ -f "$dir/pyvenv.cfg" ]]; then
        python_version=$(
          grep -E '^version[[:space:]]*=' "$dir/pyvenv.cfg" 2> /dev/null \
            | sed 's/.*=[[:space:]]*//'
        )
      fi
  ...
  if ((show_poetry)) \
    ...
    if command -v poetry > /dev/null 2>&1; then
      ...
      if poetry_env_info=$(poetry env info 2> /dev/null); then
        ...
        print "$poetry_env_info" \
          | sed 's/^/    /'
      fi

      z::log::info "  Configuration:"
      poetry config --list 2> /dev/null \
        | grep -E '^virtualenvs\.' \
        | sed 's/^/    /'
    fi
    print ""
