...
z::mod::zi::_install()
{
  ...
  if ! z::mod::zi::_check_network 3; then
  ...
  if ! command git clone --depth 1 --single-branch \
    ...
    [[ -d "$ZI_HOME" ]] \
      && command rm -rf -- "$ZI_HOME" 2> /dev/null
    return 1
  fi
...
if z::func::exists "z::mod::zi::init"; then
  z::mod::zi::init
fi
