# ============================================
# Server & Cloud Management Helper Functions & Aliases
...
# Default SSH options for better stability and security
SSH_OPTS="-o ServerAliveInterval=60 -o ConnectTimeout=10 -o StrictHostKeyChecking=accept-new"
# Options for scp commands
SCP_OPTS="-p -o ServerAliveInterval=60 -o ConnectTimeout=10 -o StrictHostKeyChecking=accept-new"
# Options for rsync commands using SSH as transport
RSYNC_OPTS="-avzP --rsh=\"ssh -o ServerAliveInterval=60 -o ConnectTimeout=10\""
...
# ============================================
# Comprehensive Server & Cloud Management Aliases (srv.*)
# ============================================
...
alias srv.ssh='ssh $SSH_OPTS'
# SSH using a jump/bastion host
# Usage: srv.ssh.via <bastion_host> <target_host>
alias srv.ssh.via='ssh $SSH_OPTS -J'
...
# Run a command on multiple hosts simultaneously (needs parallel installed)
# Usage: srv.run.parallel <hostfile> <command>
alias srv.run.parallel='_run_parallel_hosts() { parallel --tag --will-cite --no-run-if-empty -j0 --env PATH -a "$1" ssh $SSH_OPTS -o StrictHostKeyChecking=no -o BatchMode=yes {} "$2"; }; _run_parallel_hosts'

# --------------------------------------------
# File Transfer (Enhanced with jump host support)
# --------------------------------------------
# Secure copy LOCAL -> REMOTE
# Usage: srv.scp.to local/path user@host:/remote/path
alias srv.scp.to='scp $SCP_OPTS'
# Secure copy REMOTE -> LOCAL
# Usage: srv.scp.from user@host:/remote/path /local/path
alias srv.scp.from='scp $SCP_OPTS'
# Secure copy through a jump host
# Usage: srv.scp.via bastion_host user@target:/path local_path
alias srv.scp.via='_scp_via() { scp $SCP_OPTS -o "ProxyJump $1" "${@:2}"; }; _scp_via'
...
alias srv.rsync.to='rsync $RSYNC_OPTS'
...
alias srv.rsync.from='rsync $RSYNC_OPTS'
# Rsync with bwlimit (bandwidth limit in KB/s)
# Usage: srv.rsync.bwlimit 500 user@host:/remote/path/ local/path/
alias srv.rsync.bwlimit='_rsync_bwlimit() { rsync $RSYNC_OPTS --bwlimit="$1" "${@:2}"; }; _rsync_bwlimit'
...
# Find and grep files remotely (find files and search their content)
# Usage: srv.fs.findgrep <user@host> <path> <filename_pattern> <content_pattern>
alias srv.fs.findgrep='_srv_remote_exec_wrapper() { target=$1; path=$2; name_pattern=$3; content_pattern=$4; _srv_remote_exec "$target" "sudo find \"$path\" -type f -name \"$name_pattern\" -exec grep -l \"$content_pattern\" {} \\;"; }; _srv_remote_exec_wrapper'
...
# Show disk usage for largest directories/files
# Usage: srv.fs.du.top <user@host> <path> [count]
alias srv.fs.du.top='_srv_remote_exec_wrapper() { target=$1; path=$2; count=${3:-10}; _srv_remote_exec "$target" "sudo du -xh \"$path\" | sort -rh | head -n $count"; }; _srv_remote_exec_wrapper'
# Find largest files (by file size)
# Usage: srv.fs.find.large <user@host> <path> [size_threshold] [count]
alias srv.fs.find.large='_srv_remote_exec_wrapper() { target=$1; path=$2; size=${3:-+100M}; count=${4:-20}; _srv_remote_exec "$target" "sudo find \"$path\" -type f -size $size -exec ls -lh {} \\; 2>/dev/null | sort -rk5 | head -n $count"; }; _srv_remote_exec_wrapper'
# Find newest files
# Usage: srv.fs.find.new <user@host> <path> [days] [count]
alias srv.fs.find.new='_srv_remote_exec_wrapper() { target=$1; path=$2; days=${3:-7}; count=${4:-20}; _srv_remote_exec "$target" "sudo find \"$path\" -type f -mtime -$days -ls 2>/dev/null | sort -r | head -n $count"; }; _srv_remote_exec_wrapper'
...
# Usage: srv.docker <user@host> <docker_command>
alias srv.docker='_srv_remote_exec_wrapper() { target=$1; shift; _srv_remote_exec "$target" "docker $@"; }; _srv_remote_exec_wrapper'
# Interactive docker exec
# Usage: srv.docker.exec <user@host> <container_id> [command]
alias srv.docker.exec='_srv_remote_exec_interactive_wrapper() { target=$1; container=$2; cmd=${3:-sh}; _srv_remote_exec_interactive "$target" "docker exec -it $container $cmd"; }; _srv_remote_exec_interactive_wrapper'
# Docker container list (with stats)
# Usage: srv.docker.ps <user@host> [options]
alias srv.docker.ps='_srv_remote_exec_wrapper() { target=$1; shift; _srv_remote_exec "$target" "docker ps --format \"table {{.ID}}\t{{.Names}}\t{{.Image}}\t{{.Status}}\t{{.Ports}}\" $@"; }; _srv_remote_exec_wrapper'
# Docker container stats (live resource usage)
# Usage: srv.docker.stats <user@host>
alias srv.docker.stats='_srv_remote_exec_interactive_wrapper() { target=$1; _srv_remote_exec_interactive "$target" "docker stats"; }; _srv_remote_exec_interactive_wrapper'
# View logs for a specific container
# Usage: srv.docker.logs <user@host> <container_id> [options]
alias srv.docker.logs='_srv_remote_exec_wrapper() { target=$1; container=$2; shift 2; _srv_remote_exec "$target" "docker logs $@ $container"; }; _srv_remote_exec_wrapper'
# Follow container logs (live)
# Usage: srv.docker.logs.follow <user@host> <container_id>
alias srv.docker.logs.follow='_srv_remote_exec_interactive_wrapper() { target=$1; container=$2; _srv_remote_exec_interactive "$target" "docker logs -f $container"; }; _srv_remote_exec_interactive_wrapper'

# Kubernetes commands on remote host (assuming kubeconfig is set up)
# Usage: srv.kubectl <user@host> <kubectl_command>
alias srv.kubectl='_srv_remote_exec_wrapper() { target=$1; shift; _srv_remote_exec "$target" "kubectl $@"; }; _srv_remote_exec_wrapper'
# Get pods across all namespaces
# Usage: srv.kubectl.pods <user@host> [namespace]
alias srv.kubectl.pods='_srv_remote_exec_wrapper() { target=$1; ns=${2:+"-n $2"}; _srv_remote_exec "$target" "kubectl get pods $ns"; }; _srv_remote_exec_wrapper'
# Describe a specific pod
# Usage: srv.kubectl.describe <user@host> <pod_name> [namespace]
alias srv.kubectl.describe='_srv_remote_exec_wrapper() { target=$1; pod=$2; ns=${3:+"-n $3"}; _srv_remote_exec "$target" "kubectl describe pod $pod $ns"; }; _srv_remote_exec_wrapper'
# Get pod logs
# Usage: srv.kubectl.logs <user@host> <pod_name> [namespace]
alias srv.kubectl.logs='_srv_remote_exec_wrapper() { target=$1; pod=$2; ns=${3:+"-n $3"}; _srv_remote_exec "$target" "kubectl logs $pod $ns"; }; _srv_remote_exec_wrapper'
# Follow pod logs (live)
# Usage: srv.kubectl.logs.follow <user@host> <pod_name> [namespace]
alias srv.kubectl.logs.follow='_srv_remote_exec_interactive_wrapper() { target=$1; pod=$2; ns=${3:+"-n $3"}; _srv_remote_exec_interactive "$target" "kubectl logs -f $pod $ns"; }; _srv_remote_exec_interactive_wrapper'
# Execute command in a pod (interactive shell)
# Usage: srv.kubectl.exec <user@host> <pod_name> [namespace] [container] [command]
alias srv.kubectl.exec='_srv_remote_exec_interactive_wrapper() { target=$1; pod=$2; ns=${3:+"-n $3"}; container=${4:+"--container=$4"}; cmd=${5:-sh}; _srv_remote_exec_interactive "$target" "kubectl exec -it $pod $ns $container -- $cmd"; }; _srv_remote_exec_interactive_wrapper'

# --------------------------------------------
# Cloud Provider CLI Tools
# --------------------------------------------
# Execute AWS CLI commands on remote host
# Usage: srv.aws <user@host> <aws_command>
alias srv.aws='_srv_remote_exec_wrapper() { target=$1; shift; _srv_remote_exec "$target" "aws $@"; }; _srv_remote_exec_wrapper'
# Execute Azure CLI commands on remote host
# Usage: srv.az <user@host> <azure_command>
alias srv.az='_srv_remote_exec_wrapper() { target=$1; shift; _srv_remote_exec "$target" "az $@"; }; _srv_remote_exec_wrapper'
# Execute Google Cloud CLI commands on remote host
# Usage: srv.gcloud <user@host> <gcloud_command>
alias srv.gcloud='_srv_remote_exec_wrapper() { target=$1; shift; _srv_remote_exec "$target" "gcloud $@"; }; _srv_remote_exec_wrapper'

# IaC (Infrastructure as Code) Tools
# Execute Terraform commands on remote host
# Usage: srv.terraform <user@host> <terraform_command>
alias srv.terraform='_srv_remote_exec_wrapper() { target=$1; shift; _srv_remote_exec "$target" "terraform $@"; }; _srv_remote_exec_wrapper'
# Run Ansible playbook on remote control node
# Usage: srv.ansible <user@host> <playbook.yml> [extra_vars]
alias srv.ansible='_srv_remote_exec_wrapper() { target=$1; playbook=$2; shift 2; _srv_remote_exec "$target" "ansible-playbook $playbook $@"; }; _srv_remote_exec_wrapper'
# Run Ansible command on remote control node
# Usage: srv.ansible.cmd <user@host> <host_pattern> <module> [args]
alias srv.ansible.cmd='_srv_remote_exec_wrapper() { target=$1; host_pattern=$2; module=$3; shift 3; _srv_remote_exec "$target" "ansible $host_pattern -m $module $@"; }; _srv_remote_exec_wrapper'
...
srv.help() {
    ...
    echo "- Jump host syntax: user@jumphost:user@destination (e.g., admin@bastion:user@internal)"
    ...
    echo "- For batch operations on multiple hosts, use srv.run.parallel or tools like Ansible."
    echo ""
    echo "Categories:"
    echo "  - srv.info.*      Server information queries"
    echo "  - srv.fs.*        File system operations"
    echo "  - srv.net.*       Network operations from server"
    echo "  - srv.proc.*      Process management"
    echo "  - srv.log.*       Log file operations"
    echo "  - srv.mon.*       Monitoring operations"
    echo "  - srv.svc.*       Service management"
    echo "  - srv.user.*      User management"
    echo "  - srv.docker.*    Docker container management"
    echo "  - srv.kubectl.*   Kubernetes pod management"
    echo "  - srv.aws/az/gcloud Cloud provider operations"
}
