...
_net_validate_input() {
  ...
  ip)
    if [[ "$input" =~ ^([0-9]{1,3}\.){3}[0-9]{1,3}$ ]]; then
      ...
      # Improved IPv6 validation: length, colon count
      local len="${#input}"
      local colon_count="$(echo "$input" | grep -o ':' | wc -l | tr -d ' ')"
      if (( len > 39 || len < 3 || colon_count > 7 || colon_count < 2 )); then
...
_net_exec() {
  ...
  if command -v timeout >/dev/null 2>&1; then
    command timeout "$timeout_duration" "${cmd[@]}"
  elif command -v gtimeout >/dev/null 2>&1; then
    command gtimeout "$timeout_duration" "${cmd[@]}"
  else
...
_net_get_os() {
  ...
  # Check cache first
  local cached_os
  cached_os="$(awk -F= -v key="$cache_key" '$1 == key {print $2; exit}' "$cache_file" 2>/dev/null)"
  ...
  *) os_type="Unknown" ;;
  ...
  if [[ "$os_type" != "Unknown" ]]; then
    grep -v "^$cache_key=" "$cache_file" > "${cache_file}.tmp" 2>/dev/null
    echo "$cache_key=$os_type" >> "${cache_file}.tmp"
...
_net_has_tool() {
  ...
  # Check cache first
  local cached_result
  cached_result="$(awk -F= -v key="$cache_key" '$1 == key {print $2; exit}' "$cache_file" 2>/dev/null)"
  ...
  # Cache uniquely
  grep -v "^$cache_key=" "$cache_file" > "${cache_file}.tmp" 2>/dev/null
  echo "$cache_key=$result" >> "${cache_file}.tmp"
  mv "${cache_file}.tmp" "$cache_file"
